...
\label{section:dielectric_screening}
...
Moreover, Ando calculated how the optical absorption spectrum changes as the external dielectric constant increases \cite{ando2011effects}. Figure \ref{fig:energy_shift_ando} shows a key result from his study. As the ratio between the dielectric constant of the surrounding medium to that of the SWCNT increases, the optical absorption spectrum redshifts further. For a very high external dielectric constant, Coulomb interactions become screened to the extent where the aborption spectrum appears to resemble a scenario where there are no excitons.

\begin{figure}[H]
	\centering
	\includegraphics[scale=0.7]{images/chapter_optical_props/dielectric_ando_3}
	\caption{Optical absorption spectrum of a SWCNT in the scenarios where the ratio between the dielectric constant of the environment and that of the SWCNT is (a) 10 or (b) 100. The dashed curve shows absorption spectrum in the case where there are no Coulomb interactions. The energy scale is given in units of $\gamma$, which represents the electron hopping energy, and $L$ which represents the magnitude of the SWCNT chirality vector. As the dielectric constant of the surrounding medium increases, optical features redshift further. For a very large dielectric constant, the spectrum begins to resemble a scenario where there are no electron-electron interactions. Reproduced and modified from Ref.\ \cite{ando2011effects}.}
	\label{fig:energy_shift_ando}
\end{figure}
