...
It has been established that exciton binding energy varies as $E_\text{b} \propto \epsilon^{-\alpha}$ for some constant $\alpha$ \cite{perebeinos2004scaling}. Walsh \textit{et al}.\ (2007) determined that $\alpha \approx 1.2$ \cite{walsh2008scaling}. Coulomb interactions which include electron-electron and electron-hole that produce band gap renormalization and exciton binding respectively, as shown in Figure \ref{fig:schematic_screening_walsh}. Coulomb interactions can be screened by the CNT environment. For instance, this has the effect of screening the Coulomb potential between an electron and hole, thereby lowering exciton binding energy. Figure \ref{fig:coulomb_screening_walsh} shows how this Coulomb potential changes for different external dielectric constants. The potential becomes narrower for higher values of $\epsilon_2$ due to screening.
...
\begin{figure}[ht]
	...
	\caption{ A 1-D Coulomb potential function between an electron and a hole separated by a distance $z$ plotted for different values of $\varepsilon_2$, the dielectric constant of the external evironment. This function has been integrated over a CNT with lateral $x$ and $y$ lengths of 1.1 nm.  Here, $a_\text{b}^* = \epsilon_1 \hbar^2 / \mu e^2$ stands for the bulk exciton Bohr radius. As $\varepsilon_2$ increases, the potential function becomes spatially narrower due to the screening induced by the external environment. The poential function includes a cutoff parameter for $z$ to avoid the divergence as $z$ approaches zero. Reproduced from \cite{walsh2008scaling}.}
